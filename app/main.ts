        ...
        switch (method) {
            case HttpMethod.GET:
                if (path === '/') {
                    ...
                    break;
                } else if (pathParts.length > 2 && pathParts[1] === 'echo' && headers.get(HttpHeaderType.ACCEPT_ENCODING) !== null) {
                    const body = pathParts[2];
                    const responseHeaders = new Map<string, string>();
                    const encodingTypes: EncodingType[] = headers.get(HttpHeaderType.ACCEPT_ENCODING)?.split(', ') as EncodingType[] || [];
                    encodingTypes.every((encodingType) => {
                        if (Object.values(EncodingType).includes(encodingType as unknown as EncodingType)) {
                            responseHeaders.set(HttpHeaderType.CONTENT_ENCODING, encodingType);
                            return false;
                        }
                        return true;
                    });
                    ...
                    if (responseHeaders.get(HttpHeaderType.CONTENT_ENCODING) === EncodingType.GZIP) {
                        ...
                        socket.write(createResponse(HttpVersion.HTTP_1_1, HttpStatusCode.OK, responseHeaders));
